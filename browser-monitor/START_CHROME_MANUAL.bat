...
set /p USER_CHROME_PATH="Chrome路径: "

REM 移除可能的引号
set USER_CHROME_PATH=%USER_CHROME_PATH:"=%

REM 检查路径是否存在
if not exist "%USER_CHROME_PATH%" (
    echo.
    echo [错误] 找不到指定的Chrome路径：
    echo %USER_CHROME_PATH%
    echo.
    echo 请检查：
    echo   1. 路径是否正确
    echo   2. Chrome是否已安装
...
echo.
echo [1/3] 找到Chrome: %USER_CHROME_PATH%
echo.
...
start "" "%USER_CHROME_PATH%" --silent-debugger-extension-api
echo.
